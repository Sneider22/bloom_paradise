...
export default function Sidebar({ activeTab, setActiveTab, username, onLogout, isMobileOpen, setIsMobileOpen }) {
  ...
  return (
    <aside className={`w-80 bg-white border-r border-slate-200 flex flex-col justify-between h-screen lg:sticky top-0 shrink-0 select-none shadow-sm transition-transform duration-300 ease-in-out z-50 fixed lg:static inset-y-0 left-0 ${isMobileOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full lg:translate-x-0'}`}>
      
      {/* Brand Header */}
      <div>
        <div className="p-6 border-b border-slate-100 bg-gradient-to-r from-camp-green-light/45 to-transparent flex items-center justify-between">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center space-x-4 px-4 py-3.5 rounded-2xl text-left cursor-pointer transition-all duration-150 group ${
                  isActive
      ...
      <div className="p-4 border-t border-slate-100">
        {/* User Card */}
        <div className="flex items-center space-x-3 p-3 bg-slate-50 rounded-2xl border border-slate-100 mb-3">
        ...
        <button
          onClick={onLogout}
          className="w-full py-3 px-4 bg-slate-100 hover:bg-rose-50 hover:text-rose-600 text-slate-600 rounded-xl font-semibold text-xs flex items-center justify-center space-x-2 cursor-pointer transition-all border border-transparent hover:border-rose-100"
        >
